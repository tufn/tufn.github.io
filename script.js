...
const DL_MANIFESTS = {
  linux:   'downloads/linux/manifest.json',
  windows: 'downloads/windows/manifest.json'
};

async function loadDLMeta(os) {
  const metaEl = $('dl-meta');
  const btn    = $('dl-btn');
  const lbl    = $('dl-os-lbl');
  if (!os || !DL_URLS[os]) {
    if (btn) btn.style.display = 'none';
    if (metaEl) metaEl.textContent = 'Download available at launch for your platform.';
    return;
  }
  if (lbl) lbl.textContent = os === 'linux' ? 'Linux' : 'Windows';
  if (btn) btn.href = DL_URLS[os];
  try {
    const res  = await fetch(DL_MANIFESTS[os]);
    const data = await res.json();
    const parts = [];
    if (data.file)       parts.push(data.file);
    if (data.size_bytes) parts.push((data.size_bytes / 1024 / 1024).toFixed(1) + ' MB');
    if (data.sha1)       parts.push('sha1: ' + data.sha1.slice(0, 10) + '…');
